...
@section('content')
    <h1 class="text-center text-3xl uppercase font-black my-4">Listagem de Posts</h1>
    ...
    <form action="{{ route('posts.search') }}" method="post" class="bg-white">
        ...
        <div class="max-w-sm my-4 p-1 pr-0 flex items-center">
            <input type="text" name="{{ old('search') }}" placeholder="Filtrar:"
                class="flex-1 appearance-none rounded shadow p-3 text-grey-dark mr-2 focus:outline-none">
            <button type="submit"
                class="uppercase p-3 rounded bg-blue-500 text-blue-50 max-w-max shadow-sm hover:shadow-lg">Pesquisar</button>
        </div>
    </form>
                ...
                <tr>
                    ...
                    <td
                        ...
                        <a href="{{ route('posts.show', $post->id) }}"
                            class="px-5 py-2 border-blue-500 border text-blue-500 rounded transition duration-300 hover:bg-blue-700 hover:text-white focus:outline-none">Ver</a>
                        <a href="{{ route('posts.edit', $post->id) }}"
                            class="px-5 py-2 border-green-500 border text-green-500 rounded transition duration-300 hover:bg-green-700 hover:text-white focus:outline-none">Editar</a>
                    </td>
                </tr>
            @endforeach
        </tbody>

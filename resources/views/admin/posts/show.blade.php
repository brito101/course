...
@section('content')
    ...
    <div class="w-11/12 p-12 bg-white sm:w-8/12 md:w-1/2 lg:w-5/12 mx-auto">
        ...
        <form action="{{ route('posts.destroy', $post->id) }}" method="post">
            ...
            <button type="submit"
                class="w-full py-3 mt-6 font-medium tracking-widest text-white uppercase bg-red-500 shadow-lg focus:outline-none hover:bg-red-900 hover:shadow-none">Deletar
                o Post: {{ $post->title }}</button>
        </form>
    </div>
@endsection
